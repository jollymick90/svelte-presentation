<script lang="ts">
  import aboutme from "$lib/assets/img/aboutme.webp";
  import angular from "$lib/assets/img/angular.webp";
  import java from "$lib/assets/img/java.webp";
  import js from "$lib/assets/img/js.webp";
  import svelte from "$lib/assets/img/svelte.webp";
  import vue from "$lib/assets/img/vue.webp";
  import react from "$lib/assets/img/react.webp";
  import { bglightBackgroundRadial, bacaroBackground } from "../common.svelte";
  import bacarologo from "$lib/assets/img/BacaroLogo.webp";
</script>

<section>
  <section data-auto-animate data-background-gradient={bglightBackgroundRadial}>
    <div class="c-flex">
      <div class="">
        <img src={aboutme} alt="micheleimg" />
      </div>
    </div>
    <aside class="notes">io</aside>
  </section>
  <section data-auto-animate data-background-gradient={bglightBackgroundRadial}>
    <div class="c-flex">
      <div class="">
        <img src={aboutme} alt="micheleimg" />
      </div>
      <div class="c-flex c-flex-col p-2 gap-5">
        <p class="text-4xl">Michele Scarpa</p>
      </div>
    </div>
    <aside class="notes">sono michele</aside>
  </section>
  <section data-auto-animate data-background-gradient={bglightBackgroundRadial}>
    <div class="c-flex">
      <div class="">
        <img src={aboutme} alt="micheleimg" />
      </div>
      <div class="c-flex c-flex-col p-2 gap-5">
        <p class="text-4xl">Michele Scarpa</p>
        <p class="text-4xl">Software Engineer - Web Developer</p>
      </div>
    </div>
    <aside class="notes">Software Engineer e sviluppatore web</aside>
  </section>
  <section data-auto-animate data-background-gradient={bglightBackgroundRadial}>
    <div class="c-flex">
      <div class="">
        <img src={aboutme} alt="micheleimg" />
      </div>
      <div class="c-flex c-flex-col p-2 gap-5">
        <p class="text-4xl">Michele Scarpa</p>
        <p class="text-4xl">Software Engineer - Web Developer</p>
        <div class="c-flex gap-2">
          <img class="w-auto h-10" src={angular} alt="angularicon" />
          <img class="w-auto h-10" src={java} alt="javaicon" />
          <img class="w-auto h-10" src={js} alt="jsicon" />
          <img class="w-auto h-10" src={svelte} alt="svelteicon" />
          <img class="w-auto h-10" src={react} alt="reacticon" />
          <img class="w-auto h-10" src={vue} alt="vueicon" />
        </div>
      </div>
    </div>
    <aside class="notes">sono appassionato del mondo web e non solo</aside>
  </section>
  <section data-auto-animate data-background-color={bacaroBackground}>
    <div class="myHeading">
      <h1>BacaroTech</h1>
      
    </div>
    <img class="bacaroicon" src={bacarologo} alt="bacarologo" />
    <aside class="notes">
      Inoltre, Ho aiutato a fondare e faccio parte di Bacarotech
    </aside>
  </section>

  <section data-auto-animate data-background-color={bacaroBackground}>
    <div class="myHeading">
      <h1>BacaroTech</h1>
      <img class="bacaroicon" src={bacarologo} alt="bacarologo" />
    </div>

    <pre><code
        data-trim
        data-noescape
        data-line-numbers="1 | 3-6 | 8-11 | 13-16 | 18-21 | 23-25 | 28'">
  {` class Bacarotech { 

      team = [
      "Giorgio", "Michele", "Moreno", "Vittorio",
      "Lorenzo", "Davide", "Danilo", "Antonio"
      ]

      techologies = [
       'Frontend', 'Backend', 'DotNet', 'Data Eng',  
       'AI', 'DevOps', 'Kotlin', '...'
      ] 

      social = [
      'instagram', 'youtube', 
      'tiktok', 'linkedin'
      ]

      eventi = [
      'speaker', 'interviste', 
      'media partner', 'podcast'
      ]

      giveMeSomethings() {
       return {"Spritz", "Polpetta"}
      }
     }
   `}
  </code></pre>
    <aside class="notes">
      ho anche il piacere di presentarvi Bacarotech. - Cos'è? - un team di
      persone, - appassionate di diverse tecnologie, - che fanno contenuti su
      diverse piattaforme social - e partecipano a diversi eventi - il tutto
      cercando di tenere lo spirito di un luogo tipico della zona di Veneta. è
      una particolare osteria del veneziano dove vengono serviti 'ombre' (vino)
      e 'cichetti' (cibo)
    </aside>
  </section>
</section>

<style>
  :root {
    --spacing: 0.3rem;
  }

  .myHeading img {
    margin-right: 10px;
  }
  .myHeading h1,
  .myHeading img {
    display: inline-block;
    vertical-align: middle;
  }

  .c-flex {
    display: flex;
  }
  .c-flex-col {
    flex-direction: column;
  }

  .bacaroicon {
    height: 200px;
    margin-top: 0px;
  }
  .h-10 {
    height: calc(var(--spacing) * 10);
  }
  .p-2 {
    padding: calc(var(--spacing) * 2);
  }
  .gap-2 {
    gap: calc(var(--spacing) * 2);
  }
  .gap-5 {
    gap: calc(var(--spacing) * 5);
  }
  .w-auto {
    width: auto;
  }
</style>
